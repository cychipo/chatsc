...
describe('Chat group membership events e2e', () => {
    ...
    it('creates a group conversation with initial members', async () => {
        ...
        const result = await controller.createConversation({ user: { id: 'user-1' } }, { type: 'group', participantIds: ['user-2', 'user-3'], title: 'Team Chat' });
        expect(chatService.createConversation).toHaveBeenCalledWith('group', 'user-1', ['user-2', 'user-3'], 'Team Chat');
    ...
    it('adds a member to group and creates membership event', async () => {
        ...
        const result = await controller.addMember({ user: { id: 'user-1' } }, 'group-1', { userId: 'user-4' });
    ...
    it('returns already member status when adding existing member', async () => {
        ...
        const result = await controller.addMember({ user: { id: 'user-1' } }, 'group-1', { userId: 'user-2' });
...
//# sourceMappingURL=chat-group-membership-events.e2e-spec.js.map
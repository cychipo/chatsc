...
describe('Chat direct message e2e', () => {
    ...
    it('creates a direct conversation between two users', async () => {
        ...
        const result = await controller.createConversation({ user: { id: 'user-1' } }, { type: 'direct', participantIds: ['user-2'] });
    ...
    it('rejects sending message if user is not active participant', async () => {
        ...
        const encoder = new TextEncoder();
        const binaryContent = encoder.encode('Hello');
        await expect(controller.sendMessage({ user: { id: 'user-1' }, body: Buffer.from(binaryContent) }, 'conv-1')).rejects.toBeInstanceOf(common_1.ForbiddenException);
    });
});
//# sourceMappingURL=chat-direct-message.e2e-spec.js.map
...
|---|---|---|---|---|
| `mongo` | `mongo:7` | `27017` (internal) | `mongo-data:/data/db` | `unless-stopped` |
| `backend` | `Dockerfile.backend` | `5634` (exposed) | (none) | `unless-stopped` |
| `frontend` | `Dockerfile.frontend` | `5734` (exposed) | (none) | `unless-stopped` |

### Relationships

- `backend` **depends on** `mongo` (via `depends_on` + `service_healthy`)
- `frontend` **depends on** `backend` (via `depends_on`)
...
|---|---|---|---|
| `MONGODB_URI` | Auto | `mongodb://mongo:27017/chatsc` | docker-compose |
...
| `GOOGLE_CALLBACK_URL` | **YES** | `http://localhost:5634/api/auth/google/callback` | Operator fills `.env` |
| `REFRESH_COOKIE_NAME` | No | `refresh_token` | env.config.ts |
| `FRONTEND_APP_URL` | Auto | `http://localhost:5734` | docker-compose |
...
| Name | Required | Default | Source |
|---|---|---|---|
| `VITE_API_BASE_URL` | **YES** | `http://localhost:5634/api` | docker-compose ARG |
...
server {
  ...
  # No backend proxy — frontend calls backend directly via VITE_API_URL
}
```
